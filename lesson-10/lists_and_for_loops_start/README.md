...
Looping and lists go hand in hand. You can't really have one without the other. Lists normally store the data but you want to do something with each item in the list. That's where loops come in. They allow you to execute a task for each item in the list.

This is just like how you do things in real life. You get a grocery list, and when you get to the grocery store, for each item on the list you get the item. Same as a todo list, you do each task on the list. Python allows you to do the same thing.
...
```
- You can see above that we use `for` new variable name `in` list name. This is the syntax for a for loop in Python. The `for` keyword is used to start the loop.
...
```
- You can see here that we have one indent so that we know which statements are part of our `for` loop. Here there are 4 statements that are all part of the if statement.
...
- Sometimes you want to know the index of the item while you're looping over the list. There's a handy way to do this with the `enumerate` function.
...
- So what we're going to do is right after the `for` statement, we're going to check to see if the day contains the letter `u`. If it does, we're going to skip the day.
  - This also shows us that strings in Python are lists of characters. So we can use the `in` keyword to check if a character is in a string.
...
for index, day in enumerate(days_of_week):
    ...
    if day in days_of_weekend:
        ...
        print(F"{day} is a weekday")
```
- You can see that the output is much shorter because `Sunday`, `Tuesday`, `Thursday`, and `Saturday` all contain the letter `u`.
```
$ python lists_and_for_loops.py
Monday is day number 2 of the week
Monday is a weekday
...
- You already know the `%` modulo operator. You can use it the same way in a for loop to skip every other item in a list. Here we skip any day whose number is odd — `(index + 1) % 2 != 0` is true when the day number doesn't divide evenly by 2.
```python
days_of_week = ["Sunday", "Monday", "Tuesday",
                "Wednesday", "Thursday", "Friday",
                "Saturday"]
...
for index, day in enumerate(days_of_week):
    ...
    if day in days_of_weekend:
        ...
        print(F"{day} is a weekday")
```
- Coincidentally, the output is the same — because the days that have a `u` in them happen to be the same odd-numbered days. But the logic is completely different: one checks the characters in the string, the other checks the position in the list.
```
$ python lists_and_for_loops.py
Monday is day number 2 of the week
Monday is a weekday
