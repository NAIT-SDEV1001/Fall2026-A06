...
1. Open the file `pokemon_catch_broken.py` in this folder.
2. This program simulates catching a wild Charizard. The player chooses a type of Poké Ball to throw, which adds a bonus to a random roll. If the total roll meets the catch threshold, Charizard is caught.
3. The file contains both syntax errors and logical errors. Find and fix all of them so the program runs correctly.
4. Here are sample runs once fixed:
```
$ python pokemon_catch_broken.py
You encountered a wild Charizard!
...
3
Throwing Ultra Ball...
Congratulations! You caught Charizard.
```
```
$ python pokemon_catch_broken.py
You encountered a wild Charizard!
