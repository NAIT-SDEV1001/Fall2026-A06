...
### 8. Let's check if the user has a ticket and what type it is.
We've checked if the user is at the right place but we haven't checked if they have a ticket, or what kind. Let's collect that information now.
- First, ask if they have a ticket. Then, *only if they do*, ask what type.
```python
concert_name = input("What is the name of the concert tonight? ")
...
```
- Notice that `ticket_type` is only collected when the user has a ticket. If they don't have one, we don't need to know the type.
- Let's take a look at the output when the user has a ticket.
```
$ python concert_example.py
What is the name of the concert tonight? taylor swift
Do you have a ticket? (y/n) y
What type of ticket do you have? (VIP or Standard) VIP
```
- And when the user doesn't have a ticket.
```
$ python concert_example.py
What is the name of the concert tonight? taylor swift
...
### 9. Let's use the `not` operator to check if the user doesn't have a ticket.
The `not` operator inverts a boolean condition — if something is `True`, `not` makes it `False`, and vice versa. It's useful when it's clearer to say what you *don't* want rather than what you do.
- Let's restructure our if statement to check first whether the user does *not* have a ticket, then handle the concert details.
```python
concert_name = input("What is the name of the concert tonight? ")
has_ticket = input("Do you have a ticket? (y/n) ")
if has_ticket == 'y':
...
if not has_ticket == "y":
    ...
    print("this is not the concert you are looking for")
```
- `not has_ticket == "y"` is `True` whenever `has_ticket` is anything other than `"y"`. This is the same idea as writing `has_ticket != "y"`, just expressed differently.
- Notice we now check the ticket *first*. If the user doesn't have one, we tell them right away and none of the other `elif` blocks run.
- Let's look at the output when the user doesn't have a ticket.
```
$ python concert_example.py
What is the name of the concert tonight? taylor swift
Do you have a ticket? (y/n) n
...
```
- And when the user has a ticket.
```
$ python concert_example.py
What is the name of the concert tonight? taylor swift
Do you have a ticket? (y/n) y
...
### 10. Let's handle VIP and Standard tickets differently.
Right now all taylor swift ticket holders get the same message. Let's use `ticket_type` to give VIP and Standard ticket holders different experiences.
- Replace the single taylor swift `elif` with two — one for VIP and one for Standard.
```python
concert_name = input("What is the name of the concert tonight? ")
has_ticket = input("Do you have a ticket? (y/n) ")
if has_ticket == 'y':
...
if not has_ticket == "y":
    ...
    print("this is not the concert you are looking for")
```
- Let's take a look at the output for a VIP ticket.
```
$ python concert_example.py
What is the name of the concert tonight? taylor swift
Do you have a ticket? (y/n) y
...
have fun superstar!
```
- And for a Standard ticket.
```
$ python concert_example.py
What is the name of the concert tonight? taylor swift
Do you have a ticket? (y/n) y
What type of ticket do you have? (VIP or Standard) Standard
enjoy the show
```
...
We've also covered comparison operators, logical operators (`and`, `or`, `not`), and booleans. These are the building blocks of if statements. 
