...
We're going to compare numbers in this example and this is a common thing to do in programming. 
...
- Python also has three more comparison operators that you'll use frequently:
  - `!=` is the not equal operator — True when the values are *different*
  - `>=` is the greater than or equal to operator — True when the left value is greater than *or* the same as the right
  - `<=` is the less than or equal to operator — True when the left value is less than *or* the same as the right
- For example, we could rewrite the guess check to use `>=` and `<=` instead of separate `>` and `<` checks:
```python
if user_guess == random_number:
    print("You guessed the number!")
    ...
    print("lower than the number!")
```
- You can also use `!=` to catch anything that isn't equal, for example `if user_guess != random_number:` to check that the user still needs to keep guessing.
...
- We're going to use a string to store whether the guess was high, low, way off, or correct — and remove the print statements.
```python
# other code here removed for brevity
...
You are correct, and way too low!
```
