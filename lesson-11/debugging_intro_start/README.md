...
| ll | List more of the surrounding code lines|
...
- At the top of the `hi_or_low_debug.py` file add `breakpoint()` to the top of the file, put it at the same place as shown below (so we can step through it together.)
```python
import random
...
### 3. Let's start by running `l` (or `list`) in the prompt
...
### 4. Let's start by running `c` (or `continue`) in the prompt.
...
### 5. Let's go line by line running `n` (or `next`) in the prompt.
...
- You can see here that we've run a few commands in our debugger and let's talk about it.
  ...
  - In the fifth `(Pdb)` prompt we've typed `c` and pressed enter. This is going to run the rest of the code until it's done.
    - The next line of code will convert the `user_input` to an integer and assign it to `user_guess` and our code just runs until it's done.
...
### 6. Let's move our `breakpoint()` to before the `if` statement and step through it.
...
```
- Let's run our code and understand what's going on with our `if` statement in the code.
```
$ python hi_or_low_debug.py
...
- Let's take a look at what's going on here as we debug the code after our `breakpoint()`
  ...
  - third `(Pdb)` we write `l` and you can see that we're on line `17` and we haven't executed the line yet.
...
### 7. Let's change the value of `user_guess` to exactly the value of `random_number` and see how our program behaves!
- You can change the value of variables in our debugger and it's useful if you want to check a condition that is hard to reproduce. Let's take a look at how we can do that.
```
$ python hi_or_low_debug.py
...
### 8. Fix the bug in the previous step!
...
- Let's take a look at the `(Pdb)` prompts to see what's going on.
  ...
  - second `(Pdb)` we're assigning the value of `random_number` to `user_guess` so that we can test the `if` statement if the values are equal.
  - third `(Pdb)` we write `c` so that our code can continue running until it's done.
  - After the `(Pdb)`
    - We see that our code tells us it's correct! The bug is fixed!


## Tips for Effective Debugging

- Add breakpoints where you suspect issues — you don't have to put them at the top of the file.
- Check the values of variables and program flow as you step through, not just the final output.
- Use `print()` statements for quick, simple checks, but use the debugger when things get complex.
- Remove your `breakpoint()` calls when you're done — don't leave them commented out in your code.
