...
- **Shortcut:** Press `ctrl + ~` to open a new terminal in VS Code.
- in your terminal, double-check that `hello_world.py` exists by typing `ls` if you're on Mac or `dir` if you're on Windows then pressing enter. You should see something like this (on Windows):
```
...
```
- This is the "traceback". It is telling us we have an error called a `SyntaxError`. That means there is a problem with your syntax (the actual code written in the file). In this case, it's telling you that you forgot to close your parenthesis `)`. We'll learn more about this later.
- Something really important to note here is that traceback tells us the line of our code where the error is happening. This is really helpful when you're trying to figure out what's wrong with your code. So here it tells us our error is on line 2 of the file `hello_world.py`.
...
Every day in this course you will be writing code, running it, and fixing errors. This is a very important skill to learn and will help you become a better software developer.
