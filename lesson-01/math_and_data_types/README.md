...
### 2. Let's start with introducing the `string` data type.
...
- Strings are always enclosed in quotes (`" "` or `' '`).
- Notice we used the `+` operator to **concatenate** (combine) two strings together.
- Type this code into your `math_and_data_types.py` file and run it. You should see: `Hello, Alice`

We can also use f-strings to include variables in text:
```python
name = "Alice"
print(f"Hello, {name}")
```

Both approaches work! F-strings are often cleaner when you have multiple variables or want readable code.

### 3. Let's start with introducing the `int` and `float` data types.
...
days_until_assignment_due = 14        # int
...
- A few things to observe here:
  - The number `14` is an integer — no decimals, no quotes.
  - The number `25.5` is a float — it has a decimal point.
  - `days_until_assignment_due` and `assignment_completion_percentage` use ["Snake Case"](https://en.wikipedia.org/wiki/Snake_case), which is the Python convention for variable names. Every word in the variable name is lower case and separated by an underscore `_`
  - Python is **dynamically typed**, so you don't have to declare the type — just assign a value and Python figures it out.
...
Assignment completion: 25.5%
```

### 4. Let's learn about and use some math operators in Python 
...
### 5. Let's do some more math, and learn about reassigning variables.
...
### 6. More Practice with multiplication
...
# ... Subtraction and Addition code removed for brevity
...
Assignment due in 30 perceived days
```
...
### 7. More Practice with division, modulus and floats
...
### 8. Let's use the `**` operator to raise a number to a power.
...
Assignment due in 16 perceived days
```
...
### 9. Let's take a look at doing all of this math in one line.
...
Newest Assignment due in 49 perceived days
```
