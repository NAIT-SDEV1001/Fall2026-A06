...
In Python, a list is a data structure that can hold items of different data types, such as strings, integers, and floats. That said, 99.9% of the time you'll keep the same data type in a list.
...
They're used in every program that you'll write, so that's why they're important.
...
- Note you can see here that every item in the list is a string.
  - now you might be thinking yourself, it would be handy if I could access the items in that list. Well you can!
  - There's something called the "index" it starts at 0 and goes up by 1 for every item in the list.

### 3. Let's Access some of the items in the list.
...
print("the third item in our grocery list is: ")
print(groceries[2])
```
- here's the output.
```
...
the first item in our grocery list is:
lettuce
the third item in our grocery list is:
bread
```
...
```python
print(groceries[42])  # This will raise an IndexError
```
  - You'll see here that you're going to get an `IndexError` exception because there is no item at index `42` in the list.
    - We'll talk about how to handle exceptions in the coming weeks.

### 4. Let's change some items in the list.
...
```
- you can see here that if you want to change items that on the left of the `=` operator we have `groceries[0]` and on the right we assign it a new value `'romaine lettuce'`.
...
baguette
```
- Note you can see the items are changed in the list and stay changed when we access them later on.

### 5. Let's take a look at how we can "slice" items in the list.
...
### 6. Let's get the length of a list.
